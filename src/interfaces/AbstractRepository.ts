...
export default abstract class AbstractRepository{
  ...
  async create(entity: any): Promise<any> {
    try {
      const data = await this.prismaService[this.entityName].create({
        data: entity
      })
  ...
  async update(id: number, companyId: number, data: any): Promise<void> {
    const result = await this.prismaService[this.entityName].update({
      where: {
        companyId: companyId,
        id: id
      },
  ...
  async resolveIgnoredFields(data, ignoreFields?: string[]) {
